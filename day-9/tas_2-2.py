...
def edge_crosses_rectangle(
    ...
    """
    Dokładny port Elixirowego edge_crosses_rectangle?

    edge  = ((ex1, ey1), (ex2, ey2))
    rect  = ((rx1, ry1), (rx2, ry2))  – dwa przeciwległe rogi prostokąta
    """
...
def rectangle_valid(rect: Tuple[Point, Point], edges) -> bool:
    # True, jeśli żadna krawędź nie przecina wnętrza prostokąta
    for e in edges:
...
def solve_part2(points: List[Point]) -> Tuple[Tuple[Point, Point] | None, int]:
    ...
    total = len(pairs)
    # lekki progres-print co ~5%
    step = max(1, total // 20)

    for idx, (p1, p2) in enumerate(pairs):
        if idx % step == 0:
            pct = int(idx / total * 100)
            print(f"[STATUS] checked {idx}/{total} (~{pct}%), best_area={best_area}")

        # Muszą być przeciwległymi rogami prostokąta:
        x1, y1 = p1
        x2, y2 = p2
        if x1 == x2 or y1 == y2:
            continue
        ...
        if rectangle_valid(rect, edges):
            ...
            if a > best_area:
                best_area = a
                best_rect = rect
                print(f"[UPDATE] new best area={best_area}, rect={best_rect}")
...
if __name__ == "__main__":
    ...
    print(f"[INFO] Loaded {len(points)} red tiles")
    rect, a = solve_part2(points)
    print("\n=== RESULT ===")
    print(f"Largest rectangle (two red opposite corners): {rect}")
    print(f"Area: {a}")
